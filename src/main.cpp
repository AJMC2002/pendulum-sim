...
// We also consider our coordinates with respect to the position of the pivot of
// rotation
...
const double THETA_0 =
	M_PI / 4;			   // angle between the rope and the vertical [rad]
...
double modulus(double x, double y) { return std::sqrt(x * x + y * y); }
...
void write_row(std::ofstream &fout,
			   std::map<std::string, std::vector<double>> &data) {
	std::string row = "";
...
int main() {
	...
	for (double t = DT; t <= T_MAX; t = t + DT) {
		...
		data["theta"].push_back(data["theta"].back() +
								data["omega"].back() * DT +
								1 / 2 * data["alfa"].back() * DT * DT);
		data["omega"].push_back(data["omega"].back() +
								data["alfa"].back() * DT);
		...
		data["v_x"].push_back(data["omega"].back() * data["L"].back() *
							  cos(data["theta"].back()));
		data["v_y"].push_back(data["omega"].back() * data["L"].back() *
							  sin(data["theta"].back()));
		data["||v||"].push_back(
			modulus(data["v_x"].back(), data["v_y"].back()));

		data["v_x_noisy"].push_back(filter.noisify(data["v_x"].back()));
		data["v_y_noisy"].push_back(filter.noisify(data["v_y"].back()));

		data["a_x"].push_back(data["alfa"].back() * data["L"].back() *
							  cos(data["theta"].back()));
		data["a_y"].push_back(data["alfa"].back() * data["L"].back() *
							  sin(data["theta"].back()));
		data["||a||"].push_back(
			modulus(data["a_x"].back(), data["a_y"].back()));
	}
